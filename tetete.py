        ...
        if self.highlighted_block:
            ...
            if isinstance(self.highlighted_block, ControlBlock) and self.highlighted_block.is_open_area(self.scenePos()):
                ...
                if self.dragging_from_top and self.is_near(self.highlighted_block, above=True):
                    ...
                    # Align positions
                    new_x = self.highlighted_block.scenePos().x() - abs(self.width - self.highlighted_block.width) // 2
...
class StartBlock(Block):
    ...
    def generate_code(self):
        if self.next_block:
            return self.next_block.generate_code()
        return ""
...
class PINBlock(Block):
    ...
    def generate_code(self):
        ...
        if self.combo_box.currentText() == "ON":
            ...
            text = "LOW"
        program = program.format(self.text_field.text(), text)
        if self.next_block:
